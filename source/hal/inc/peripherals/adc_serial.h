...
public:
    constexpr AdcSerial(adc_serial_regs volatile& base) noexcept
        : m_regs(base), m_data_width(0) {}

    void set_config(int cpol, int cpha, int cnv_pol, int cnv_with_cs, int busy_pol,
                    int busy_src, int busy_time, int clk_act_in_cnv, int data_width) {
        int val = 0;
